...
/// Ring buffer SPSC per processo (PID → (req_phys, resp_phys)). Allocata
/// dalla syscall `SYS_RING_ALLOC`; (0, 0) = mai allocata. Single-core, i PID
/// non vengono riutilizzati → nessuna race, nessun free necessario.
const RING_MAX_PROCS: usize = 128;
/// Coppie (req_ring_phys, resp_ring_phys) per PID. Slot dispari = req, pari = resp.
static mut RING_PHYS: [u64; RING_MAX_PROCS * 2] = [0; RING_MAX_PROCS * 2];
...
/// Alloca (o ritorna se gia' allocata) le due pagine ring del processo `pid`.
/// Ritorna `(req_phys, resp_phys)` oppure `None` se mancano frame. Le pagine
/// vengono zero-fill. Il mapping nello spazio del processo e' a carico del
/// chiamante (syscall).
pub fn alloc_ring_pages(pid: usize) -> Option<(u64, u64)> {
    if pid < RING_MAX_PROCS {
        let req = unsafe { *core::ptr::addr_of!(RING_PHYS[pid * 2]) };
        let resp = unsafe { *core::ptr::addr_of!(RING_PHYS[pid * 2 + 1]) };
        if req != 0 && resp != 0 {
    ...
    if pid < RING_MAX_PROCS {
        unsafe {
            *core::ptr::addr_of_mut!(RING_PHYS[pid * 2]) = req;
            *core::ptr::addr_of_mut!(RING_PHYS[pid * 2 + 1]) = resp;
        }
    }
    Some((req, resp))
}
...
pub unsafe fn teardown_user_space(cr3: u64, pid: usize) {
    ...
    if pid < RING_MAX_PROCS {
        unsafe {
            *core::ptr::addr_of_mut!(RING_PHYS[pid * 2]) = 0;
            *core::ptr::addr_of_mut!(RING_PHYS[pid * 2 + 1]) = 0;
        }
    }
}
