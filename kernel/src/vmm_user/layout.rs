...
/// Bit NX sulla PTE (bit 63, Fase M1): richiede EFER.NXE (abilitato in
...
/// Flag foglia per pagine dati user (M1): RW + NX. Usato da tutti i mapping
/// foglia tranne il codice (unico eseguibile) e i livelli tabella.
pub(super) const USER_LEAF_RW: u64 = 0x4 | 0x3 | PTE_NX; // U + P + W + NX
/// Flag foglia read-only (M1): per VMA PROT_READ materializzate lazy.
...
/// Single source in `syscall-numbers` (M1: serve anche ai test per la guard).
pub use syscall_numbers::USER_STACK_TOP;
/// Numero di frame (4 KiB) dello stack user.
pub use syscall_numbers::USER_STACK_FRAMES;
/// Pagina guard sotto lo stack (M1): mai mappata, vedi `syscall-numbers`.
