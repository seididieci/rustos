...
impl GrantTable {
    ...
    pub fn insert(&mut self, g: Grant) -> u64 {
        ...
        loop {
            ...
            n ^= mix;
            // Mai 0 (riservato) e mai nell'intervallo sentinelle ERR..=ERR_INVALID
            // (`!0-8..=!0`): un nonce indistinguibile da un rifiuto farebbe
            // fallire il claim sul client (`fs_reply_check` mappa le sentinelle
            // in `Err` prima ancora di usarle come handle).
            if n == 0 || n >= !0u64 - 8 || self.grants.contains_key(&n) {
                continue;
            }
    ...
    pub fn purge_registrant(&mut self, chan: u64) {
        self.grants.retain(|_, g| g.registrant_chan != chan);
    }
}
...
pub fn handle_cancel(grants: &mut GrantTable, _chan: u64, nonce: u64) -> Result<u64, u64> {
    grants.remove(nonce);
    Ok(0)
}
